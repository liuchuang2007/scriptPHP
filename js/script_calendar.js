...
function loadcalendar() {
	...
	s += '<tr id="hourminute"><td colspan="7" align="center" style="display: none;"><input type="text" size="2" value="" id="hour" class="txt" onKeyUp=\'this.value=this.value > 23 ? 23 : zerofill(this.value);controlid.value=controlid.value.replace(/\\d+(\:\\d+)/ig, this.value+"$1")\'> 点 <input type="text" size="2" value="" id="minute" class="txt" onKeyUp=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'> 分</td></tr>';
	s += '</table></div></div>';
	...
	for(var k = 2020; k >= 1931; k--) {
		...
		s += '<a href="###" onclick="refreshcalendar(' + k + ', mm);dm_id(\'calendar_year\').style.display=\'none\'"><span' + (today.getFullYear() == k ? ' class="calendar_today"' : '') + ' id="calendar_year_' + k + '">' + k + '</span></a><br />';
	}
	s += '</div></div>';
	s += '<div id="calendar_month" onclick="doane(event)" style="display: none;z-index:100001;">';
	for(var k = 1; k <= 12; k++) {
		s += '<a href="###" onclick="refreshcalendar(yy, ' + (k - 1) + ');dm_id(\'calendar_month\').style.display=\'none\'"><span' + (today.getMonth()+1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' 月</span></a><br />';
	}
	s += '</div>';
	if(is_ie && is_ie < 7) {
	...
	div.innerHTML = s;
	dm_id('append_parent').appendChild(div);
	document.onclick = function(event) {
		dm_id('calendar').style.display = 'none';
		dm_id('calendar_year').style.display = 'none';
		dm_id('calendar_month').style.display = 'none';
		if(is_ie && is_ie < 7) {
			dm_id('calendariframe').style.display = 'none';
			dm_id('calendariframe_year').style.display = 'none';
			dm_id('calendariframe_month').style.display = 'none';
		}
	}
	dm_id('calendar').onclick = function(event) {
		doane(event);
		dm_id('calendar_year').style.display = 'none';
		dm_id('calendar_month').style.display = 'none';
		if(is_ie && is_ie < 7) {
...
function settime(d) {
	...
	controlid.value = yy + "-" + zerofill(mm + 1) + "-" + zerofill(d) + (addtime ? ' ' + zerofill(dm_id('hour').value) + ':' + zerofill(dm_id('minute').value) : '');
}
...
function showcalendar(event, controlid1, addtime1, startdate1, enddate1) {
	...
	dm_id('calendar').style.display = 'block';	
	dm_id('calendar').style.left = p['x']+'px';
	dm_id('calendar').style.top	= (p['y'] + 20)+'px';
	
	doane(event);
	refreshcalendar(currday.getFullYear(), currday.getMonth());
	if(lastcheckedyear != false) {
		dm_id('calendar_year_' + lastcheckedyear).className = 'calendar_default';
		dm_id('calendar_year_' + today.getFullYear()).className = 'calendar_today';
	}
	if(lastcheckedmonth != false) {
		dm_id('calendar_month_' + lastcheckedmonth).className = 'calendar_default';
		dm_id('calendar_month_' + (today.getMonth() + 1)).className = 'calendar_today';
	}
	dm_id('calendar_year_' + currday.getFullYear()).className = 'calendar_checked';
	dm_id('calendar_month_' + (currday.getMonth() + 1)).className = 'calendar_checked';
	//dm_id('hourminute').style.display = addtime ? '' : 'none';
	lastcheckedyear = currday.getFullYear();
	lastcheckedmonth = currday.getMonth() + 1;
	if(is_ie && is_ie < 7) {
		dm_id('calendariframe').style.top = dm_id('calendar').style.top;
		dm_id('calendariframe').style.left = dm_id('calendar').style.left;
		dm_id('calendariframe').style.width = dm_id('calendar').offsetWidth;
		dm_id('calendariframe').style.height = dm_id('calendar').offsetHeight;
		dm_id('calendariframe').style.display = 'block';
	}
}
...
function refreshcalendar(y, m) {
	...
	yy = x.getFullYear();
	mm = x.getMonth();
	dm_id("year").innerHTML = yy;
	dm_id("month").innerHTML = mm + 1 > 9  ? (mm + 1) : '0' + (mm + 1);
	...
	if(addtime) {
		dm_id('hour').value = zerofill(hh);
		dm_id('minute').value = zerofill(ii);
	}
}

function showdiv(id) {
	var p = getposition(dm_id(id));
	dm_id('calendar_' + id).style.left = p['x']+'px';
	dm_id('calendar_' + id).style.top = (p['y'] + 16)+'px';
	dm_id('calendar_' + id).style.display = 'block';
	if(is_ie && is_ie < 7) {
		dm_id('calendariframe_' + id).style.top = dm_id('calendar_' + id).style.top;
		dm_id('calendariframe_' + id).style.left = dm_id('calendar_' + id).style.left;
		dm_id('calendariframe_' + id).style.width = dm_id('calendar_' + id).offsetWidth;
		dm_id('calendariframe_' + id ).style.height = dm_id('calendar_' + id).offsetHeight;
		dm_id('calendariframe_' + id).style.display = 'block';
	}
}
