<?php
class RegionModel extends AgentDB {
    ...
    public function getSupplierStat($type,$param='') {
        ...
        if ($type == 'sdlist') {//suppliers list in specified day.
            ...
            $total['date'] = date('Y/m/d',$time);
            $btype = BRANCH_TYPE;
            $branches = $this->query("select a.dcode,name from department a,manager b where a.dcode = b.dcode and pregion='{$depart['dcode']}' and type = $btype group by a.dcode",'DEFAULT','all');
            ...
            if ($stype == 'lweek' || $stype == 'lmonth' || $stype == 'onemonth') {
                ...
                foreach($time as $item) {
                    $sql = "select sum(total)as total,sum(suppliermoney) as suppliermoney,sum(storemoney) as storemoney,dtime from (select scode from supplier where pregion='{$depart['dcode']}') a, busirecord b where b.scode = a.scode and b.status = 2 and dtime = $item";
                    $row = $this->query($sql,'DEFAULT','assoc');
                    $row['date'] = date('Y/m/d',$item);
                    $row['dtime'] = $item;
                    $row['total'] = empty($row['total']) ? 0 : $row['total'];
                    $row['return'] = $activity->calculate($row['suppliermoney'], $row['storemoney']);
                    $sql = "select sum(total)as total from (select scode from supplier where pregion='{$depart['dcode']}') a, busirecord b where b.scode = a.scode and b.status = 3 and dtime = '$item'";
                    $cancel = $this->query($sql,'DEFAULT','assoc');
                    $row['cancel'] = empty($cancel['total']) ? 0 : $cancel['total'];
                ...
                foreach($time as $item) {
                    ...
                    pregion='{$depart['dcode']}') a, busirecord b where b.scode = a.scode and b.status = 2 and dtime >= '{$item['start']}' and dtime <= '{$item['end']}'";
                    $row = $this->query($sql,'DEFAULT','assoc');
                    $row['date'] = date('Y年m月',$item['start']);
                    $row['dtime'] = $item['start'];
                    $row['total'] = empty($row['total']) ? 0 : $row['total'];
                    $row['return'] = $activity->calculate($row['suppliermoney'], $row['storemoney']);
                    $sql = "select sum(total)as total from (select scode from supplier where pregion='{$depart['dcode']}') a, busirecord b where b.scode = a.scode and b.status = 3 and dtime >= '{$item['start']}' and dtime <= '{$item['end']}'";
                    $cancel = $this->query($sql,'DEFAULT','assoc');
                    $row['cancel'] = empty($cancel['total']) ? 0 : $cancel['total'];
    ...
    public function getOperationStat($type,$param='') {
        ...
        if ($type == 'search') {
            ...
            $sql = "select a.name,a.dcode from department a, manager b where a.dcode = b.dcode and pregion = '{$depart['dcode']}' and type = $branchtype group by a.dcode";
    ...
    public function getCancelStat($type,$param='') {
        ...
        if ($type == 'branchlist') {
            ...
            if ($stype == 'lweek' || $stype == 'lmonth'||$stype == 'onemonth') {
                ...
                foreach($time as $item) {
                    $sql = "select sum(total)as total,sum(suppliermoney) as suppliermoney,sum(storemoney) as storemoney,dtime from (select dcode from department where pregion='{$depart['dcode']}') a, busirecord b where b.storecode = a.dcode and b.status = 2 and dtime = $item";
                    $row = $this->query($sql,'DEFAULT','assoc');
                    $row['date'] = date('Y/m/d',$item);
                    $row['dtime'] = $item;
                    $row['total'] = empty($row['total']) ? 0 : $row['total'];
                    $row['return'] = $activity->calculate($row['suppliermoney'], $row['storemoney']);
                    $sql = "select sum(total)as total from (select dcode from department where pregion='{$depart['dcode']}') a, busirecord b where b.storecode = a.dcode and b.status = 3 and dtime = '$item'";
                    $cancel = $this->query($sql,'DEFAULT','assoc');
                    $row['cancel'] = empty($cancel['total']) ? 0 : $cancel['total'];
                ...
                foreach($time as $item) {
                    ...
                    pregion='{$depart['dcode']}') a, busirecord b where b.storecode = a.dcode and b.status = 2 and dtime >= '{$item['start']}' and dtime <= '{$item['end']}'";
                    $row = $this->query($sql,'DEFAULT','assoc');
                    $row['date'] = date('Y年m月',$item['start']);
                    $row['dtime'] = $item['start'];
                    $row['total'] = empty($row['total']) ? 0 : $row['total'];
                    $row['return'] = $activity->calculate($row['suppliermoney'], $row['storemoney']);
                    $sql = "select sum(total)as total from (select dcode from department where pregion='{$depart['dcode']}') a, busirecord b where b.storecode = a.dcode and b.status = 3 and dtime >= '{$item['start']}' and dtime <= '{$item['end']}'";
                    $cancel = $this->query($sql,'DEFAULT','assoc');
                    $row['cancel'] = empty($cancel['total']) ? 0 : $cancel['total'];
